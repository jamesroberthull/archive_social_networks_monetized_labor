********************************************************************
**     Program Name: /home/jrhull/diss/ch2/c2prog/c2_00_14.sas
...
**        1.) RAW DATA
**     Output Data:
**        1.) /trainee/jrhull/diss/ch2/c2data/c2_00_14.xpt
**
...
libname ot&y.&f.01 xport '/trainee/jrhull/diss/ch2/c2data/c2_0002B.xpt';
libname ot&y.&f.02 xport '/trainee/jrhull/diss/ch2/c2data/c2_0003B.xpt';
libname ot&y.&f.03 xport '/trainee/jrhull/diss/ch2/c2data/c2_00HHB.xpt';
libname ot&y.&f.04 xport '/trainee/jrhull/diss/ch2/c2data/c2_0007B.xpt';
libname ot&y.&f.05 xport '/trainee/jrhull/diss/ch2/c2data/c2_0008B.xpt';
libname ot&y.&f.06 xport '/trainee/jrhull/diss/ch2/c2data/c2_0009B.xpt';
libname ot&y.&f.07 xport '/trainee/jrhull/diss/ch2/c2data/c2_0010B.xpt';
libname ot&y.&f.08 xport '/trainee/jrhull/diss/ch2/c2data/c2_0012B.xpt';
libname ot&y.&f.09 xport '/trainee/jrhull/diss/ch2/c2data/c2_0013B.xpt';
...
data ot&y.&f.02.c2_0003B;
     set work&y.&f.25;
run;

data ot&y&f.03.c2_00HHB;
     set work&y.&f.18;
run;
...
data work&y.&f.50 (keep=HHID00 RICE_YLD);
     set in&y.&f.01.hh00 (keep=HHID00 X6_87A: rename=(HHID00=HHID00C));
...
*** Determine the quadrants for land and labor ***;

proc univariate data=work&y.&f.52;
     var RICE_YLD;
     output out=junk pctlpts=50 pctlpre=p;
run;
...
data work&y.&f.53;
     ...
     if RICE_YLD=. then YLD_TERT=4;
        else if RICE_YLD=<333.33 then YLD_TERT=1;
        else if RICE_YLD>333.33 and RICE_YLD=<613.33 then YLD_TERT=2;
        else if RICE_YLD>613.33 then YLD_TERT=3;

     if H_TOT_T=. then LAB_TERT=4;
        else if H_TOT_T=<4 then LAB_TERT=1;
        else if H_TOT_T>4 and H_TOT_T=<8 then LAB_TERT=2;
        else if H_TOT_T>8 then LAB_TERT=3;

     if RICE_YLD=. then YLD_QUAD=3;
        else if RICE_YLD=<466.67 then YLD_QUAD=1;
        else if RICE_YLD>466.67 then YLD_QUAD=2;

     if H_TOT_T=. then LAB_QUAD=3;
        else if H_TOT_T=<5 then LAB_QUAD=1;
        else if H_TOT_T>5 then LAB_QUAD=2;
...
data work&y.&f.54;
     ...
     if YLD_QUAD=1 and LAB_QUAD=1 then CORR_B_2=1;
     if YLD_QUAD=1 and LAB_QUAD=2 then CORR_B_2=2;
     if YLD_QUAD=2 and LAB_QUAD=1 then CORR_B_2=3;
     if YLD_QUAD=2 and LAB_QUAD=2 then CORR_B_2=4;

run;

proc freq;
     tables CORR_BOX CORR_B_2;
run;
...
proc sort data=work&y.&f.54 out=work&y.&f.56;
     by CORR_B_2 HHID&y;
run;

proc corr data=work&y.&f.56;
     var  H_PF_01 H_PF_11 H_PF_10;
     with H_OI_01 H_OI_11 H_OI_10
          HG_RSR&y HG_ROR&y HG_RIR&y H&y.RPCNT H&y.RPAVG
          HG_RSS&y HG_ROS&y HG_RIS&y H&y.SPCNT H&y.SPAVG;
...
****************************************************************************
** ALTERNATIVE Version of Specificity Tests Using Village-Level Variables **
****************************************************************************;

libname in&y.&f.10 xport '/trainee/jrhull/diss/ch2/c2data/c3_00_VL.xpt';
libname in&y.&f.11 xport '/nangrong/data_sas/1994/current/comm94.01';

data work&y.&f.60;
     set in&y.&f.10.C3_&y._VL;
run;

data work&y.&f.61;
     set in&y.&f.11.comm94 (keep= VILL94 VILL84);
      if VILL84 ne 98;
run;

proc sort data=work&y.&f.61 out=work&y.&f.62;
     by VILL94;
run;

data work&y.&f.63;
     merge work&y.&f.60 (in=a)
           work&y.&f.62 (in=b);
     by VILL94;
run;

proc sort data=work&y.&f.63 out=work&y.&f.64;
     by VILL84;
run;

data work&y.&f.65 (keep=V84 VILL1355 VILL_RIC);
     set work&y.&f.64;
     V84=input(VILL84, best12.);
run;


data work&y.&f.66 (keep=V84 V_1355 V_RICE);
     set work&y.&f.65;

     by V84;

     retain V_1355 V_RICE;
     ...
     V_1355=V_1355+VILL1355;
     V_RICE=V_RICE+VILL_RIC;

     if last.V84 then output;

run;

*** Determine the village-level quadrants for land and labor ***;

proc univariate data=work&y.&f.66;
     var V_1355 V_RICE;
     output out=junk pctlpts=50 pctlpre=lab_ pctlpre=land_;
run;

proc print;
run;                   ** 50% labor = 640 50% land = 2153 **;


data work&y.&f.67;
     set work&y.&f.66;

     if V_RICE=. then V_LND_QD=3;
        else if V_RICE=<2153 then V_LND_QD=1;
        else if V_RICE>2153 then V_LND_QD=2;

     if V_1355=. then V_LAB_QD=3;
        else if V_1355=<640 then V_LAB_QD=1;
        else if V_1355>640 then V_LAB_QD=2;

run;

proc freq;
     tables V_LND_QD V_LAB_QD;
run;

data work&y.&f.68;
     set work&y.&f.67;

     if V_LND_QD=1 and V_LAB_QD=1 then CORR_B_V=1;
     if V_LND_QD=1 and V_LAB_QD=2 then CORR_B_V=2;
     if V_LND_QD=2 and V_LAB_QD=1 then CORR_B_V=3;
     if V_LND_QD=2 and V_LAB_QD=2 then CORR_B_V=4;

run;

** Merge new Village-Level data with HH-level file **;

data work&y.&f.69 (drop=v84C);
     set ot&y.&f.08.c2_&y.12B (rename=(V84=V84C));  *** A shortcut, when running entire script, should be work&y.&f.45 **;

     V84=input(V84C, best12.);
run;

proc sort data=work&y.&f.69 out=work&y.&f.70;
     by V84;
run;

data work&y.&f.71;
     merge work&y.&f.70 (in=a)
           work&y.&f.68 (in=b);
     by V84;
     if a=1 then output;
run;

proc sort data=work&y.&f.71 out=work&y.&f.72;
     by CORR_B_V;
run;

proc corr data=work&y.&f.72;
     var  H_PF_01 H_PF_11 H_PF_10;
     with H_OI_01 H_OI_11 H_OI_10
          HG_RSR&y HG_ROR&y HG_RIR&y H&y.RPCNT H&y.RPAVG
          HG_RSS&y HG_ROS&y HG_RIS&y H&y.SPCNT H&y.SPAVG;
...
** Alternative Specification of Village Grouping Variable **;

proc univariate data=work&y.&f.72;
     var V_1355 V_RICE;
     output out=junk pctlpts=50 pctlpre=lab_ pctlpre=land_;
run;

proc print;
run;                   ** 50% labor = 665 50% land = 2499 **;

data work&y.&f.73;
     set work&y.&f.72;

     if V_RICE=. then V_LND_Q2=3;
        else if V_RICE=<2499 then V_LND_Q2=1;
        else if V_RICE>2499 then V_LND_Q2=2;

     if V_1355=. then V_LAB_Q2=3;
        else if V_1355=<665 then V_LAB_Q2=1;
        else if V_1355>665 then V_LAB_Q2=2;
...
data work&y.&f.74;
     ...
     if V_LND_Q2=1 and V_LAB_Q2=1 then CORR_BV2=1;
     if V_LND_Q2=1 and V_LAB_Q2=2 then CORR_BV2=2;
     if V_LND_Q2=2 and V_LAB_Q2=1 then CORR_BV2=3;
     if V_LND_Q2=2 and V_LAB_Q2=2 then CORR_BV2=4;

run;

proc sort data=work&y.&f.74 out=work&y.&f.75;
     by CORR_BV2;
run;

proc corr data=work&y.&f.75;
     var  H_PF_01 H_PF_11 H_PF_10;
     with H_OI_01 H_OI_11 H_OI_10
          HG_RSR&y HG_ROR&y HG_RIR&y H&y.RPCNT H&y.RPAVG
          HG_RSS&y HG_ROS&y HG_RIS&y H&y.SPCNT H&y.SPAVG;
...
proc freq;
     tables V_LND_QD V_LND_Q2 V_LAB_QD V_LAB_Q2 CORR_B_V CORR_BV2;
run;

**********************************************
** Specificity test by number of HH Members **
**********************************************;

** Create HH Member Count Variables **;

data work&y.&f.80 (keep=HHID00 CEP00 X1 AGE X4 X25 X28);
     set in&y.&f.03.indiv00 (keep=HHID00 CEP00 X1 X3 X4 X25 X28 CODE2 rename=(HHID00=HHID00C));
...
data work&y.&f.81 (keep=HHID00 NUMMEMS NUM_1355);
     set work&y.&f.80;

     by HHID00;

     retain NUMMEMS NUM_1355;
     ...
     if X1 ^in (0,2,3,9) then do;
                            ...
                            if (13 <= AGE <=55) then NUM_1355=NUM_1355+1;
...
data work&y.&f.82;
     set work&y.&f.54;  ** Bring in from earlier **;
run;

data work&y.&f.83;
     merge work&y.&f.81 (in=a)
           work&y.&f.82 (in=b);
     by HHID&y;
     if b=1 then output;
run;
...
proc univariate data=work&y.&f.83;
     var NUMMEMS NUM_1355;
     output out=junk pctlpts=50 pctlpre=mem_a pctlpre=mem_w;
run;

proc print;
     format mem_a50 mem_w50 8.2;
run;                   ** ALL 50% = WORKING AGE 50% = **;

data work&y.&f.84;
     set work&y.&f.83;

     if NUMMEMS=. then MEM_QUAD=3;
        else if NUMMEMS=<4 then MEM_QUAD=1;
        else if NUMMEMS>4 then MEM_QUAD=2;

     if NUM_1355=. then WRK_QUAD=3;
        else if NUM_1355=<2 then WRK_QUAD=1;
        else if NUM_1355>2 then WRK_QUAD=2;
...
data work&y.&f.85;
     ...
     if YLD_QUAD=1 and MEM_QUAD=1 then CORR_B_3=1;
     if YLD_QUAD=1 and MEM_QUAD=2 then CORR_B_3=2;
     if YLD_QUAD=2 and MEM_QUAD=1 then CORR_B_3=3;
     if YLD_QUAD=2 and MEM_QUAD=2 then CORR_B_3=4;


     if YLD_QUAD=1 and WRK_QUAD=1 then CORR_B_4=1;
     if YLD_QUAD=1 and WRK_QUAD=2 then CORR_B_4=2;
     if YLD_QUAD=2 and WRK_QUAD=1 then CORR_B_4=3;
     if YLD_QUAD=2 and WRK_QUAD=2 then CORR_B_4=4;

run;

proc freq;
     tables CORR_B_3 CORR_B_4;
run;


proc sort data=work&y.&f.85 out=work&y.&f.86;
     by CORR_B_3;
run;

proc corr data=work&y.&f.86;
     var  H_PF_01 H_PF_11 H_PF_10;
     with H_OI_01 H_OI_11 H_OI_10
          HG_RSR&y HG_ROR&y HG_RIR&y H&y.RPCNT H&y.RPAVG
          HG_RSS&y HG_ROS&y HG_RIS&y H&y.SPCNT H&y.SPAVG;
...
proc sort data=work&y.&f.85 out=work&y.&f.87;
     by CORR_B_4;
run;

proc corr data=work&y.&f.87;
     var  H_PF_01 H_PF_11 H_PF_10;
     with H_OI_01 H_OI_11 H_OI_10
          HG_RSR&y HG_ROR&y HG_RIR&y H&y.RPCNT H&y.RPAVG
          HG_RSS&y HG_ROS&y HG_RIS&y H&y.SPCNT H&y.SPAVG;
...
***************************************
** Additional Village-Level Analyses **
***************************************;

data work&y.&f.90;
     set ot&y.&f.07.c2_&y.10B;  ** TEMP FIX: should be work&y.&f.44 **;
run;

data work&y.&f.91;
     merge work&y.&f.90 (in=a)
           work&y.&f.68 (in=b);
     by V84;
...
data work&y.&f.92;
     ...
     RICE_RAT=V_RICE/V_1355;
run;

proc corr data=work&y.&f.92;
     var RICE_RAT;
     with VH_PX_PD VH_PX_FR VH_PR_PD VH_PR_FR VH_PH_PD VH_PH_FR V_PRO_PD V_PRO_FR;
run;


libname ot&y.&f.11 xport '/trainee/jrhull/diss/ch2/c2data/c2_00_RL.xpt';

data ot&y.&f.11.c2_&y._RL;
     set work&y.&f.92;
run;
